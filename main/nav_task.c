...
static void nav_tick(const vfh_config_t *vfh_cfg)
{
    ...
    if (nav.state == NAV_STUCK) {
        /* STUCK: hold position, wait, then retry when the path clears */
        uint32_t now_ms = (uint32_t)(esp_timer_get_time() / 1000);
        if ((now_ms - nav.stuck_since_ms) >= NAV_STUCK_HOLD_MS) {
            if (free_count >= NAV_STUCK_MIN_FREE) {
                ESP_LOGI(TAG, "Path cleared (%d free bins) — retrying", free_count);
                new_state    = NAV_ROTATING;
                new_steering = 0.0f;
            } else {
                /* Still blocked — reset timer and wait again */
                new_stuck_since_ms = now_ms;
                ESP_LOGW(TAG, "Still stuck (%d free bins)", free_count);
            }
        }
        /* Hold setpoint was already issued when we entered STUCK */

    } else if (free_count == 0) {
        /* All VFH bins blocked — declare stuck */
        mavlink_set_hold();
        new_state          = NAV_STUCK;
        new_stuck_since_ms = (uint32_t)(esp_timer_get_time() / 1000);
        new_stuck_count    = nav.stuck_count + 1;
        ESP_LOGW(TAG, "STUCK — all directions blocked (event #%lu)", (unsigned long)new_stuck_count);

    } else {
        /* Normal navigation: get VFH steering */
        float steering = vfh_compute(&scan, vfh_cfg, goal_body_angle, nav.prev_steering_rad);
